...
parser = argparse.ArgumentParser(description='Compact FER')

parser.add_argument('--root_path', type=str, default='../data/RAFDB/Image/aligned/', help='path to root path of images')
parser.add_argument('--database', type=str, default='RAFDB', help='Which Database for train. (RAFDB, Flatcam, FERPLUS)')
parser.add_argument('--train_list', type=str, default = '../data/RAFDB/EmoLabel/train_label.txt', help='path to training list')
...
parser.add_argument('--resume', default='', type=str, metavar='PATH',   help='path to latest checkpoint (default: checkpoints_rafdb/best_of_all_checkpoint.pth.tar)')
...
def main():
    ...
    train_dataset = ImageList(root=args.root_path, fileList=args.train_list, transform=train_transform)
    ...
    test_data = ImageList(root=args.root_path, fileList=args.test_list, transform=valid_transform)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            torch.save(  {
                         ...
                         }, os.path.join(args.model_dir, args.train_list.split('/')[-1]+'best_checkpoint.pth.tar'))
...
def train(train_loader,  basemodel, criterion, optimizer, epoch):
    ...
    for i, (input1,input2, target) in enumerate(train_loader):
        ...
        avg_predictions = torch.mean(region_preds, dim=2) #average of predictions from local and global regions
