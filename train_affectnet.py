...
#from model_without_pretrained import FERNet as net, count_parameters
from model import FERNet as net, count_parameters
from dataset.sampler import ImbalancedDatasetSampler
#from dataset.affectnet_dataset_mirror import ImageList #use for affectnet
#from dataset.fedro_dataset import ImageList as TestList

from dataset.noisy_affectnet_dataset_mirror import ImageList #use this for noisy training

os.environ['CUDA_VISIBLE_DEVICES'] = '0'
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

parser = argparse.ArgumentParser(description='Imponderous FER')
parser.add_argument('--root_path', type=str, default='../data/AffectNetdataset/Manually_Annotated_Images_aligned/',  help='path to root path of images')
parser.add_argument('--database', type=str, default='AffectNet', help='Which Database for train. (RAFDB, Flatcam, FERPLUS)')
parser.add_argument('--train_list', type=str, default ='../data/Affectnetmetadata/training.csv', help='path to training list')
parser.add_argument('--test_list', type=str, default ='../data/Affectnetmetadata/validation.csv', help='path to test list,')                                                             
#parser.add_argument('--test_list', type=str, default ='../data/FED_RO/occlusion_emotion_fedro_our_list.txt', help='path to test list, validation.csv,                                                              occlusion_affectnet_list.txt,pose_45_affectnet_list.txt, ../data/FED_RO/occlusion_emotion_fedro_our_list.txt')
...
parser.add_argument('--model_dir','-m', default='checkpoints_affectnet', type=str)
parser.add_argument('--imagesize', type=int, default = 128, help='image size (default: 224)')
...
def main():
    ...
    basemodel = net(num_classes= args.num_classes, num_regions = args.num_regions).to(device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            torch.save(  {
                         ...
                         }, os.path.join(args.model_dir,args.train_list.split('/')[-1][:-3]+'best_checkpoint.pth.tar'))
