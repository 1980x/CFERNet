...
#from dataset.ferplus_dataset_mirror import ImageList #Use this for standard clean benchmark FPLus
from dataset.noisy_fplus_dataset_mirror import ImageList #use this for noisy training
...
parser.add_argument('--database', type=str, default='FERPLUS', help='Which Database for train. (RAFDB, Flatcam, FERPLUS)')
parser.add_argument('--train_list', type=str, default ='../data/FERPLUS/Dataset/Labels/FER2013TrainValid/label.csv', help='path to training list')
parser.add_argument('--test_list', type=str, default ='../data/FERPLUS/Dataset/Labels/FER2013Test/label.csv', help='path to test list')
#occlusion_list.txt, pose_30_list.txt, pose_45_list.txt, label.csv
...
parser.add_argument('--resume', default='', type=str, metavar='PATH',   help='path to latest checkpoint (default: none checkpoints_fplus/best_of_all_checkpoint.pth.tar)')
parser.add_argument('--pretrained', default='', type=str, metavar='PATH', help='path to latest checkpoint (default: none)')
parser.add_argument('--model_dir','-m', default='checkpoints_fplus', type=str)
...
def main():
    ...
    test_dataset = ImageList(root=args.root_path+'Images/FER2013Test/', fileList=args.test_list,
    ...
    # prepare model
    basemodel = net(num_classes= args.num_classes, num_regions = args.num_regions)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            torch.save(  {
                         ...
                         }, os.path.join(args.model_dir, args.train_list.split('/')[-1][:-3]+'best_checkpoint_fplus.pth.tar'))
