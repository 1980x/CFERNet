...
from model_only_lightcnnbase import FERNet as net, count_parameters  
...
parser.add_argument('--test_list', type=str, default ='../data/RAFDB/EmoLabel/rafdb_occlusion_list.txt',
                    help='path to test list test_label.txt')
...
parser.add_argument('--resume', default='checkpoints_rafdb/best_checkpoint_rafdb_baseline.pth.tar', type=str, metavar='PATH',   help='path to latest checkpoint (default: none)')
parser.add_argument('--pretrained', default='', type=str, metavar='PATH', help='path to latest checkpoint (default: none, pretrained/LightCNN_29Layers_V2_checkpoint.pth.tar)')
...
def main():
    ...
    if args.pretrained:
       ...
       [pretrained_state_dict.pop(key) for key in keys if (('fc' in key) or ('fc2' in key))]   # for light cnn 29 v2    
       new_dict = dict(zip(list(basemodel.state_dict().keys()), list(pretrained_state_dict.values())))
       basemodel.load_state_dict(new_dict, strict = False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            ...
            torch.save(  {
                         ...
                         }, os.path.join(args.model_dir, 'best_checkpoint_rafdb_baseline.pth.tar'))
